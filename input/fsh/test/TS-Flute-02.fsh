...
* fixture[=].resource = Reference(DocumentReference/EXP-S2-ExcludedPatient)
...
* test.action[=].assert.description = "Confirm that the returned Parameters resource contains an 'isIncluded' parameter with a value of false."
* test.action[=].assert.direction = #response
* test.action[=].assert.expression = "Parameters.parameter.where(name = 'isIncluded' ).value=false"
* test.action[=].assert.warningOnly = false
